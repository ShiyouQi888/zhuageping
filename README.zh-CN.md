...
- 版本：`0.1.17`
- 平台：Windows x64
- 安装包：`zhuageping-Setup-0.1.17-x64.exe`
- SHA256：`A071DB093DA0629655332F27453B3E479F41DA61B4CCFC60FBB7E7143AAA20F5`
- Release：[抓个屏 v0.1.17](https://github.com/ShiyouQi888/zhuageping/releases/tag/v0.1.17)
...
- 截图与区域录屏新增顶层窗口快速感知：悬停预览窗口、单击直接选中，也可拖动自定义矩形区域。
- 新增多显示器感知覆盖层和逐屏 DPI 坐标换算，鼠标跨屏时会自动清除旧屏幕残留的感知框。
- 安装包内置 NativeAOT Windows 窗口探测器，实测启动耗时由约 1 秒降低到约 30–50 毫秒，无需用户安装额外运行环境。
- 全屏录制恢复真实完整显示器尺寸，不再为控制条预留或裁掉顶部和四周像素。
- 选区覆盖层与原生 Desktop Duplication 录制彻底分离，修复自定义矩形录屏黑屏问题。
- 全屏录制在预览和倒计时阶段显示向内绘制的边框，正式录制前隐藏覆盖层，边框不会进入视频。
- 提升双显示器下 MP4/H.264 原生录制稳定性，并继续支持电脑声音、可选麦克风、鼠标、点击提示、画质和帧率配置。
